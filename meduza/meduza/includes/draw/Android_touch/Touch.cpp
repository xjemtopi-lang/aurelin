                ...
                if (event.code == SYN_REPORT && imgui_io) {
#if TOUCH_MODE == 2
                    bool down = _input.trackingIDPresent ? isDown : event_readed_count > 2;
#elif TOUCH_MODE == 0
                    bool down = isDown;
#else
                    ...
                    if (down) {
                        ...
                        switch (orientation) {
                            case 1:
                                mx = y;
                                my = _input.absX.maximum - x;
                                break;
                            case 2:
                                mx = _input.absX.maximum - x;
                                my = _input.absY.maximum - y;
                                break;
                            case 3:
                                mx = _input.absY.maximum - y;
                                my = x;
                                break;
                            default:
                                mx = x;
                        ...
                        mx *= _input.absXMultiplier;
                        my *= _input.absYMultiplier;
                        ...
                        if (scroll_enabled) {
                            if (!dragging) {
                                ...
                                if (!scrolling) {
                                    accX += fabsf(ddx);
                                    accY += fabsf(ddy);
                                    if (accY > 12.0f && accY > accX * 1.3f) {
                                        scrolling = true;
                                        scrollY = my;
                                    }
                                }
                                if (scrolling) {
                                    imgui_io->MouseWheel += (scrollY - my) * 0.06f;
                                    scrollY = my;
                                    down = false;
                                }
                            }
                        ...
                        imgui_io->MouseDown[0] = down;
                        imgui_io->MousePos.x = mx;
                        imgui_io->MousePos.y = my;
                    } else {
                        dragging = false;
                        scrolling = false;
